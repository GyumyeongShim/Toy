...
public class EffectManager : FMonoSingleton<EffectManager>
{
            ...
            default:
                {
                    ...
                    for (int i = 0; i < listHitEffParam.Count; i++)
                    {
                        ...
                        if (param.UnitType > 0)
                        {
                            ...
                            else if (param.UnitType == (int)eCharacterType.Supporter && mListCheckSupporterId.Count > 0)
                            {
                                if (mListCheckSupporterId.Find(x => x == param.UnitTableId) == 0)
                                {
                                    continue;
                                }
                            }
    ...
    public ParticleSystem GetEffectOrNull(int tableId, eType type)
    {
        //sEffInfo effInfo = mListEffInfo.Find(x => x.id == tableId && x.type == type);
    ...
    public float Play(Unit parent, int tableId, eType type, float endTime = 0.0f, bool skipRegistStop = false, bool keepOnDie = false)
    {
        ...
        Transform bone = parent.aniEvent ? parent.aniEvent.GetBoneByName(effInfo.attach) : null;
    ...
    public float Play(GameObject parent, int tableId, eType type)
    {
        sEffInfo effInfo = mListEffInfo.Find(x => x.id == tableId && x.type == type);// && !x.ps.gameObject.activeSelf);
        if (effInfo == null)
        {
    ...
    public void StopAll()
    {
        ...
        {
            sParticleInfo info = mListParticle[i];
            if (info == null || info.Particle == null)
            {
                continue;
            }
    ...
    private void SetCheckList()
    {
        ...
        if (World.Instance.StageType == eSTAGETYPE.STAGE_PVP)
        {
            ...
            if (worldPVP)
            {
                for (int i = 0; i < worldPVP.PlayerChars.Length; i++)
                {
                    ...
                    {
                        mListCheckSupporterId.Add(player.boSupporter.data.TableID);
                    }
                }
        ...
        else
        {
            ...
            {
                mListCheckPlayerId.Add(World.Instance.Player.tableId);

                if (World.Instance.Player.boSupporter != null && World.Instance.Player.boSupporter.data != null)
                {
                    mListCheckSupporterId.Add(World.Instance.Player.boSupporter.data.TableID);
                }
            }
            else
            {
    ...
    private sParticleInfo GetParticleInfoOrNull(string name, eParticleFindType findType)
    {
        ...
        {
            //sParticleInfo particleInfo = mListParticle.Find(x => x.Name.CompareTo(name) == 0);
        ...
        {
            //sParticleInfo particleInfo = mListParticle.Find(x => x.Name.CompareTo(name) == 0 && x.Particle.gameObject.activeSelf);
            sParticleInfo particleInfo = null;
            for (int i = 0; i < mListParticle.Count; i++)
            {
        ...
        {
            //List<sParticleInfo> listParticleInfos = mListParticle.FindAll(x => x.Name.CompareTo(name) == 0);// && !x.Particle.gameObject.activeSelf);
    ...
    private IEnumerator UpdateMove(sEffInfo effInfo, sParticleInfo particleInfo, Unit startUnit, Unit target, int endEffectId, eType endEffectType)
    {
        ...
        Vector3 v1 = ((startUnit.transform.position - target.transform.position) + Vector3.up).normalized;
